...
float4 Edge(v2f i)
{
fixed4 sum = abs(getPixel(2, 2, i) - getPixel(2, -2, i));
sum += abs(getPixel(2, 2, i) - getPixel(-2, 2, i));
sum = abs(getPixel(3, 3, i) - getPixel(3, -3, i));
sum += abs(getPixel(3, 3, i) - getPixel(-3, 3, i));
sum *= 0.5;
return length(sum);			
}
...
txt = lerp(float4(0,0,0,0),txt,Edge(i));
