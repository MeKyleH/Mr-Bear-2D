...
vt_rnd += 1.5 * rand(trunc(uv.yy + t, float2(7,7)));
vt_rnd = vt_rnd * 2.0 - 1.0;
vt_rnd = sign(vt_rnd) * sat( ( abs(vt_rnd) -0.6) / (0.4) );
vt_rnd = lerp(0, vt_rnd, _Glitch);
float2 uv_nm = uv;
...
sample_yuv.y /= 1.0-3.0 * abs(vt_rnd) * sat( 10.5 - vt_rnd);
sample_yuv.z += 1.125 * vt_rnd * sat( vt_rnd - 0.5);
sample = float4( yuv2rgb(sample_yuv), sample.a);
float factor = rand(i.texcoord.xy*(_TimeX*100));
float4 finalColor = lerp(sample, float4(0.0,0,0.0,1), factor/2);
tx = lerp(tx, finalColor*2, _Noise);
return tx;
}
