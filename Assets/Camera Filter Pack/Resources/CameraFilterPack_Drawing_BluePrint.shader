...
{
float4 f = tex2D(_MainTex, i.texcoord);
float3 paper = tex2D(_MainTex2,i.texcoord).rgb;
float ce = 1;
float4 tex1[4];
float4 tex2[4]; 
float tex=_Value1;
...
float3 paper2 = tex2D(_MainTex2,i.texcoord+dist).rgb;
tex2[0] = tex2D(_MainTex, i.texcoord+float2(tex,0)+dist/128);
tex2[1] = tex2D(_MainTex, i.texcoord+float2(-tex,0)+dist/128);
tex2[2] = tex2D(_MainTex, i.texcoord+float2(0,tex)+dist/128);
tex2[3] = tex2D(_MainTex, i.texcoord+float2(0,-tex)+dist/128);
...
paper=lerp(float3(0.1+paper.r*0.175,0.1+paper.r*0.402,0.1+paper.r*0.687),_PColor2.rgb,_Value6); 
