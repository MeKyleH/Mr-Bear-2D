...
fixed4 color2 = tex2D(_MainTex, normalizedCoord2 + float2(dist, dist));
dist += (color.x*color2.y*0.5*normalizedCoord2.x)* _Distortion;

float y =
0.7*sin((normalizedCoord3.y + _TimeX) * 4.0) * 0.038 +
0.3*sin((normalizedCoord3.y + _TimeX) * 8.0) * 0.010 +
0.05*sin((normalizedCoord3.y + _TimeX) * 40.0) * 0.05;

float x =
0.5*sin((normalizedCoord3.y + _TimeX) * 5.0) * 0.1 +
...
fixed4 color3 = tex2D(_MainTex, normalizedCoord3+float2(dist,dist));
fixed4 MemoColor = color3;
fixed4 finalColor = lerp( lerp(color, color2, lerp(_Colored-0.2, _Colored+0.2, unitDrunk1)), color3, 0.6);
